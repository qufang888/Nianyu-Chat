...
export default function App() {
  ...
  // 通知卡片点击跳转：主进程经 app:openChat 下发目标位置。
  // chatType==='moments' 表示跳朋友圈；其余为普通聊天跳转。
  useEffect(() => {
    if (!api?.onOpenChat) return;
    return api.onOpenChat((chat: { chatType: string; chatId: string; name?: string }) => {
      if (!chat || !chat.chatType) return;
      if (chat.chatType === 'moments') {
        setView('moments');
        return;
      }
      setView('chats');
      void openChat(chat.chatType, chat.chatId, chat.name || '');
    });
