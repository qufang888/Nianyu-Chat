...
const GUIDE: GuideSection[] = [
  ...
  },
  {
    ...
    body: (
      <>
        <p>在语音设置填写独立的 TTS 专用 API 后，AI 回复可自动朗读。</p>
        <ul>
          <li>「按角色配置音色」：为每位数字人单独指定音色，未指定则回退全局默认。</li>
          <li>音色列表优先从服务端拉取（/audio/voices），不支持时回退内置清单（alloy/echo/fable/onyx/nova/shimmer），也可手动输入。</li>
          <li>开启「全局自动播报」后每条 AI 回复自动播放；群聊按发言角色匹配音色。</li>
        </ul>
      </>
    ),
  },
