...
// 随机事件快捷主题见 ../eventThemes（主窗/小窗共用）
  ...
  const speak = async (msg: ChatMessage) => {
    ...
    try {
      ...
      const src = await api.textToSpeech(msg.content, ttsRoleId);
          ...
          <>
            {(!isUser && msg.reasoning) || hasText ? (
              <div className={`bubble ${failed ? 'bubble-failed' : ''}`} onMouseUp={handleMouseUp}>
                ...
                {hasText && renderMarkdown(msg.content, { citations: citeCitations, onCite: citeOnClick, onCiteMiss: citeOnMiss })}
