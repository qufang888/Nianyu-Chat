...
export const StatsView: React.FC = () => {
  ...
  const [roles, setRoles] = useState<Record<string, { avatar: string; affinity: number; mood: string }>>({});

  const refresh = () => {
    api.getRoleStats().then(setStats);
    api.getGlobalTokens().then(setGlobal);
    api.getRoles().then((rs: Role[]) => {
      const map: Record<string, { avatar: string; affinity: number; mood: string }> = {};
      for (const r of rs) {
        map[r.id] = { avatar: r.avatar_path, affinity: r.affinity || 0, mood: r.mood || '' };
      }
      setRoles(map);
    });
  };
